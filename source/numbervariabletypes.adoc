...
Great Cow BASIC implements support for Bit, Byte, Word and Long Variable Types, all of which are described below, but Great Cow BASIC does not yet support Long Long (64 Bit).
...
*Byte* is the most common variable size in 8 Bit devices and could represent a Number, an ASCII Character, a Port, two Nibbles, an Internal Register or any user defined collection of to eight Bits such as a group of flags.
